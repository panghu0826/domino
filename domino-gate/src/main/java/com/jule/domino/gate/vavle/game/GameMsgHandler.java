 ...
 */
public class GameMsgHandler extends SimpleChannelInboundHandler<ByteBuf> {
    private final static Logger logger = LoggerFactory.getLogger(GameMsgHandler.class);
    private GameConnectPool gameConnectPool;

    public GameMsgHandler(GameConnectPool gameConnectPool) {
        this.gameConnectPool = gameConnectPool;
    }

    @Override
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        logger.debug("777-------------------------------:"+this.gameConnectPool.canAdd());
        if (this.gameConnectPool.canAdd()) {
            logger.debug("--------------------ace: OK");
