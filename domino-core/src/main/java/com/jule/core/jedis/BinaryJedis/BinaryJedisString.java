...
import static com.jule.core.jedis.BinaryJedis.BinaryJedisGeneral.OK;
...
public class BinaryJedisString extends StoredObjCodec {
     ...
     * @return 执行 INCR 命令之后 key 的值。
     */
    public static long incrBy(int dbId, String key, long stepNum) {
        JedisPool pool = JedisFactory.getPool();
        BinaryJedis redis = null;
        try {
            redis = pool.getResource();
            redis.select(dbId);
            return redis.incrBy(encode(key), stepNum);//对存储在指定key的数值执行原子的加1操作。
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            return 0;
        } finally {
     ...
     * @return 执行命令之后 key 的值。
     */
    public static long decrBy(int dbId, String key, long stepNum) {
        JedisPool pool = JedisFactory.getPool();
        BinaryJedis redis = null;
        try {
            redis = pool.getResource();
            redis.select(dbId);
            return redis.decrBy(encode(key), stepNum);
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            return 0;
        } finally {
     ...
     * @return 执行 INCR 命令之后 key 的值。
     */
    public static double incrByFloat(int dbId, String key, double stepNum) {
        JedisPool pool = JedisFactory.getPool();
        BinaryJedis redis = null;
        try {
            redis = pool.getResource();
            redis.select(dbId);
            return redis.incrByFloat(encode(key), stepNum);//对存储在指定key的数值执行原子的加1操作。
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            return 0;
        } finally {
