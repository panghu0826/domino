...
public class ProductionService {
    ...
    private static Map<Integer, JoloGame.JoloGame_ItemInfo> GIFT_ITEM_MAP = new HashMap<>(); //所有礼物的配置
    private static Map<Integer, JoloGame.JoloGame_ItemInfo> LIMIT_GIFT_ITEM_MAP = new HashMap<>(); //所有礼物的配置
    ...
    public boolean discoverData() {
        ...
        List<ItemConfigBean> allTemplates = new ArrayList<>();
        if (allTemplates == null || allTemplates.size()<=0) {
            logger.error("discoverData(),Did not connected item server");
            return false;
        }
        allTemplates.forEach(item -> {
            if (item.getItemType() == ItemType.TYPE_NPC || item.getItemType() == ItemType.TYPE_GIFT ||
                    item.getItemType() == ItemType.TYPE_NPC_COMMON || item.getItemType() == ItemType.TYPE_GIFT_COMMON) {
                GIFT_CONFIG.put(item.getId(), item);
                commonItemsMap.put(item.getId(), item);

                GIFT_ITEM_MAP.put(item.getId(), JoloGame.JoloGame_ItemInfo.newBuilder().setItemPrice(item.getPrice())
        ...
        if (list != null && list.size() != 0) {
            ...
            list.forEach(e -> {
                ...
                if (item != null) {
                    ...
                    LIMIT_GIFT_ITEM_MAP.put(item.getId(), JoloGame.JoloGame_ItemInfo.newBuilder().setItemPrice(item.getPrice())
                            .setItemExtraPrice(item.getExtraPrice())
                            .setItemDesc(item.getDetails()).setItemIcon(item.getItemIcon()).setItemId("" + item.getId()).setItemName(item.getItemName())
    ...
    public Collection<JoloGame.JoloGame_ItemInfo> getLimitGiftConfig() {
